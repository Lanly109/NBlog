...
| ---------- | ---------------------------------- | ---------------------------------- | --------------- | ---------- | -------------------------- |
| 单元测试 | 部署ip                           | 后端接收到部署请求和ip           | 2022.05.23前  | 梁励     | 监听端口的后端           |
| 单元测试 | 博文编辑                         | 后端接收到保存请求和最新博文数据 | 2022.05.23前  | 梁励     | 监听端口的后端           |
| 单元测试 | 用户点击博文一览                 | 后端接收到请求博文列表请求       | 2022.05.23前  | 梁励     | 监听端口的后端           |
| 单元测试 | 用户点击某篇博文                 | 后端接收到请求具体博文内容的请求 | 2022.05.23前  | 张涛麟   | 监听端口的后端           |
| 单元测试 | 用户删除某篇博文                 | 后端接收到删除某篇博文的请求     | 2022.05.23前  | 张涛麟   | 监听端口的后端           |
| 单元测试 | 获取用户信息                     | 后端接收到请求用户信息的请求     | 2022.05.23前  | 张涛麟   | 监听端口的后端           |
| 单元测试 | 更新用户信息                     | 后端接收到更新用户信息的请求     | 2022.05.23前  | 张涛麟   | 监听端口的后端           |
| 集成测试 | 小白部署编写更新博文的一系列行为 | 后端能依次正确接收到对应请求     | 2022.05.25前  | 黄海宇   | 监听端口的后端           |
...
| 技术要求 | 输入数据                         | 预期结果                               | 进度安排      | 人员职责 | 设备条件驱动程序和桩模块 |
| ---------- | ---------------------------------- | ---------------------------------------- | --------------- | ---------- | -------------------------- |
| 单元测试 | 部署行为                         | 部署机子成功部署博客且能正常访问       | 2022.05.24前  | 黄海宇   | 监听端口的前端           |
| 单元测试 | 博文编辑                         | 博文信息成功更新且告知前端成功         | 2022.05.24前  | 黄海宇   | 监听端口的前端           |
| 单元测试 | 用户点击博文一览                 | 后端返回博文列表信息给前端             | 2022.05.24前  | 黄海宇   | 监听端口的前端           |
| 单元测试 | 用户点击某篇博文                 | 后端返回某篇博文信息给前端             | 2022.05.24前  | 黄海宇   | 监听端口的前端           |
| 单元测试 | 用户删除某篇博文                 | 后端删除某篇博文的数据且告知前端成功   | 2022.05.24前  | 张涛麟   | 监听端口的前端           |
| 单元测试 | 获取用户信息                     | 后端返回用户信息给前端                 | 2022.05.24前  | 张涛麟   | 监听端口的前端           |
| 单元测试 | 更新用户信息                     | 后端更新用户信息且告知前端成功         | 2022.05.24前  | 张涛麟   | 监听端口的前端           |
| 集成测试 | 小白部署编写更新博文的一系列行为 | 后端能依次正确处理到对应请求并给予回应 | 2022.05.26前  | 梁励     | 监听端口的前端           |
